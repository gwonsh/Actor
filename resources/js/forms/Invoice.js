Ext.define('resources.js.forms.Quotation', {
	...
	setHtml:function(values, viewer){
		...
		/* Invoice ID*/
		html +=  	 '<div style="width:100%;height:60px;border-bottom:2px solid #000;background-color:'+baseColor+'">';
		...
		html += 		'<span class="label-left-space" style="font-size:40px;line-height:150%;color:black;font-weight:bold">'+categoryName+'</span>';
		html +=      '</div>';

		html +=		 '<table cellpadding="6" cellspacing="0" width="100%" style="border-bottom: 1px solid #333333">';
		html +=      	'<tr>';//Issue date
		html +=      		'<td valign="top" width="40%">';
		html +=             	'<strong>' + dtVal.colsName + ': </strong><span '+dtVal.cls+'>' + dtVal.dataVal + '</span><br>';
		html +=             	'<strong>' + ivcNo.colsName + ': </strong><span>' + ivcNo.dataVal + '</span><br>';
		html +=             	'<strong>' + payment.colsName + ': </strong><span '+payment.cls+'>' + payment.dataVal + '</span><br>';
		html +=             	'<strong>' + attn.colsName + ': </strong><span '+attn.cls+'>' + attn.dataVal + '</span><br>';
		html +=      		'</td>';
		html +=      		'<td id="customerInfo"'+clientInfo.cls+'>';//customer info
		html +=      		'</td>';
		html +=      	'</tr>';
		html +=		 '</table>';
		...
		html +=		 '<table cellpadding="6" cellspacing="0" width="100%" style="margin-top:43px">'
		html +=		 	'<tr>';
		html +=		 		'<td><strong>'+etc.colsName+'</strong></td>';
		html +=		 	'</tr>';
		html +=		 	'<tr>';
		html +=		 		'<td>'+etc.dataVal.replace(/\n/g, "<br />")+'</td>';
		html +=		 	'</tr>';
		html +=      '</table>';
		html += '</div>';
		viewer.setHtml(html);
		viewer.fireEvent('formcomplete');
